      ...
      <div id="col8" class="content-bg clearfix boxShadow">
        <div id="col4" class="main-left">
          <div id="aside" class="pad">
            <dl class="nav-left mt10 border-no">
              ...
              <dd <?php if(($_GET["type"])  !=  "send"): ?>class="current"<?php endif; ?>>
                <a href="<?php echo U('public/Comment/index',array('app_name'=>t($_GET['app_name']),'type'=>'receive'));?>">收到的评论</a>
              </dd>
              <dd <?php if(($_GET["type"])  ==  "send"): ?>class="current"<?php endif; ?>>
                <a href="<?php echo U('public/Comment/index',array('app_name'=>t($_GET['app_name']),'type'=>'send'));?>">发出的评论</a>
              </dd>
        ...
        <div id="col7" class="main-right">
          <div class="extend minh person-set">
            <div class="feed-nav">
              <div class="set-hd">
                <h3><?php if(($_GET["type"])  !=  "send"): ?><?php echo L('PUBLIC_RECEIVE_COMMENT');?><?php endif; ?><?php if(($_GET["type"])  ==  "send"): ?><?php echo L('PUBLIC_SEND_COMMENT');?><?php endif; ?></h3>
              </div>
              <div class="mod-feed-tab">
                <ul class="inner-feed-nav">
                  <li <?php if(($_GET["t"])  ==  ""): ?>class="current"<?php endif; ?>>
                    <a href="<?php echo U('public/Comment',array('type'=>t($_GET['type'])));?>"><?php echo L('PUBLIC_ALL_STREAM');?></a>
                  </li>
                  <?php if(is_array($tab)): ?><?php $i = 0;?><?php $__LIST__ = $tab?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$t): ?><?php ++$i;?><?php $mod = ($i % 2 )?><?php if($tabHash[$key]){ ?>
                      <li <?php if(($_GET["t"])  ==  $key): ?>class="current"<?php endif; ?>>
                        <a href="<?php echo U('public/Comment',array('t'=>$key,'type'=>t($_GET['type'])));?>"><?php echo ($tabHash[$key]); ?></a>
                      </li>
                    <?php } ?><?php endforeach; ?><?php endif; ?><?php else: echo "" ;?><?php endif; ?>
                </ul>
              </div>
            </div>
            <?php $cancomment1 = intval(CheckPermission('core_normal','feed_comment')); ?>
            <!-- 收到的评论 -->
            <?php if($_GET['type'] == 'receive'): ?>
            <div class="feed_lists" id="comment-lists">
              <?php if(($list["data"])  !=  ""): ?><?php if(is_array($list["data"])): ?><?php $i = 0;?><?php $__LIST__ = $list["data"]?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><dl class="feed_list clearfix" id="comment_list" model-node='comment_list'>
                <dt class="face"><a href="<?php echo ($vo["user_info"]["space_url"]); ?>"><img src="<?php echo ($vo["user_info"]["avatar_small"]); ?>" width="50" height="50"></a></dt>
                <dd class="content">
                  <?php $source_user_at = '@{uid='.$vo['sourceInfo']['source_user_info']['uid'].'|uname='.$vo['sourceInfo']['source_user_info']['uname'].'}：';?>
                  <?php if($vo['sourceInfo']['feedtype'] == 'post' || $vo['sourceInfo']['feedtype'] == 'weiba_post'): ?>
                  <p class="hd"><?php echo getUserSpace($vo["user_info"]["uid"],'','','{uname}') ?>：<?php echo ($vo["content"]); ?>
                  </p>
                  <dl class="comment">
                    <dt class="arrow bgcolor_arrow"><em class="arrline">◆</em><span class="downline">◆</span></dt>
                    <dd class="name"><?php echo getUserSpace($vo["sourceInfo"]["source_user_info"]["uid"],'','','@{uname}') ?></dd>
                    <dd><?php echo (format($vo["sourceInfo"]["source_body"],true)); ?></dd>
                  </dl>
                  <?php elseif($vo['sourceInfo']['feedtype'] == 'repost'): ?>
                  <p class="hd"><?php echo getUserSpace($vo["user_info"]["uid"],'','','{uname}') ?>：<?php echo ($vo["content"]); ?>//<?php echo (parse_html($source_user_at)); ?><?php echo (format($vo["sourceInfo"]["source_body"],true)); ?>
                  </p>
                  <?php else: ?>
                  <p class="hd"><?php echo getUserSpace($vo["user_info"]["uid"],'','','{uname}') ?>：
                    <?php if($vo['is_audit']){ ?>
                      <?php echo ($vo["content"]); ?>
                    <?php }else{ ?>
                      内容正在审核
                  ...
                  <p class="info">
                    <?php if($vo['is_audit']){ ?>
                      <span class="right">
                      <?php if(CheckPermission('core_normal','comment_del')){ ?>
                        <em class="hover">
                          <a event-args="comment_id=<?php echo ($vo["comment_id"]); ?>" event-node="comment_del" href="javascript:void(0);"><?php echo L('PUBLIC_STREAM_DELETE');?></a>
                          <i class="vline">|</i>
                        </em>
                        <?php } ?>
                        <a href="<?php echo ($vo["sourceInfo"]["source_url"]); ?>"><?php echo L('PUBLIC_VIEW');?></a>
                        <?php if(in_array('comment',$weibo_premission)): ?>
                        <?php if(($_GET["type"])  !=  "send"): ?><i class="vline">|</i><a href="javascript:void(0);" onclick="if(<?php echo ($cancomment1); ?>){ui.reply(<?php echo $vo['comment_id']; ?>);}else{ui.error('没有权限');}"><?php echo L('PUBLIC_STREAM_REPLY');?></a><?php endif; ?>
                        <?php endif; ?>
                      </span>
                      <span class="date"><?php echo (friendlydate($vo["ctime"])); ?></span>&nbsp;
                      <span>来自网站</span>
                      <?php if($i<=$unread_comment_count){ ?>
              ...
              <?php else: ?>
              <div class="note"><?php echo L('PUBLIC_COMMENTS_ISNOT');?></div><?php endif; ?>
              <div id='page' class='page'><?php echo ($list["html"]); ?></div>
            </div>
            <?php else: ?>
            <!-- 发出的评论 -->
            <div class="feed_lists" id="feed-lists">
              <?php if(($list["data"])  !=  ""): ?><?php if(is_array($list["data"])): ?><?php $i = 0;?><?php $__LIST__ = $list["data"]?><?php if( count($__LIST__)==0 ) : echo "" ; ?><?php else: ?><?php foreach($__LIST__ as $key=>$vo): ?><?php ++$i;?><?php $mod = ($i % 2 )?><dl class="feed_list" id="comment_list" model-node='comment_list'>
                <dt class="face"><a href="<?php echo ($vo["user_info"]["space_url"]); ?>"><img src="<?php echo ($vo["user_info"]["avatar_small"]); ?>"></a></dt>
                <dd class="content">
                  <?php $source_user_at = '@{uid='.$vo['sourceInfo']['source_user_info']['uid'].'|uname='.$vo['sourceInfo']['source_user_info']['uname'].'}：';?>
                  <?php if($vo['sourceInfo']['feedtype'] == 'post' || $vo['sourceInfo']['feedtype'] == 'weiba_post'): ?>
                  <p class="hd"><?php echo getUserSpace($vo["user_info"]["uid"],'','','{uname}') ?>：<?php echo ($vo["content"]); ?><?php echo (parse_html($vo["replyInfo"])); ?></p>
                  <dl class="comment">
                    <dt class="arrow bgcolor_arrow"><em class="arrline">◆</em><span class="downline">◆</span></dt>
                    <dd class="name"><?php echo ($vo["sourceInfo"]["source_user_info"]["space_link"]); ?></dd>
                    <dd><?php echo (format($vo["sourceInfo"]["source_body"],true)); ?></dd>
                  </dl>
                  <?php elseif($vo['sourceInfo']['feedtype'] == 'repost'): ?>
                  <p class="hd"><?php echo getUserSpace($vo["user_info"]["uid"],'','','{uname}') ?>：<?php echo ($vo["content"]); ?>//<?php echo (parse_html($source_user_at)); ?><?php echo (format($vo["sourceInfo"]["source_body"],true)); ?></p>
                  <?php else: ?>
                  <p class="hd"><?php echo getUserSpace($vo["user_info"]["uid"],'','','{uname}') ?>：<?php echo ($vo["content"]); ?></p>
                  <dl class="comment">
                    <dt class="arrow bgcolor_arrow"><em class="arrline">◆</em><span class="downline">◆</span></dt>
                    <dd class="name"><?php echo getUserSpace($vo["sourceInfo"]["source_user_info"]["uid"],'','','@{uname}') ?></dd>
                    <dd><?php echo (format($vo["sourceInfo"]["source_body"],true)); ?></dd>
...
M.addEventFns({
  ...
  comment_del:{
    click:function(){
      ...
      ui.confirm(this,'<?php echo L('PUBLIC_COMMENT_CONFIRM_TIPS');?>',function(msg){
        $(pmodel).fadeOut();
